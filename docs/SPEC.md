...
- **名称过滤**：Tab 栏右侧搜索框，输入即筛（忽略大小写），匹配**文件名或抽屉路径**：
  抽屉名命中则其全部直属文件随行可见（后代路径天然含祖先路径 -> 整个子树命中；
  反向仅名中含关键字的子分支透出）；搜索中无命中内容的分组隐藏（「全部」的空抽屉
  占位也临时隐藏），Esc 清除；输入框聚焦时 C# 的 Ctrl+C/V/Del 检测不拦截（正常编辑）
...
- **悬浮预览气泡**：悬停文件卡片 ≥800ms -> **卡片旁小气泡**（约 410×520，锚定卡片
  左侧/右侧、垂直居中夹在面板内）；**纯展示不可交互**（pointer-events 穿透，不挡鼠标），
  鼠标离开卡片 150ms 即隐，滑向相邻卡片 120ms 快速切换；选择模式下不预览。
  图片/视频（自动静音循环，无控制条）/PDF 直载 files.local；**文本经 C# `readText`
  读回**（编码探测：BOM -> 严格 UTF-8 -> GB18030 回退，中文记事本 ANSI 不乱码；
  128KB 截断；回执校验当前预览项防串台）。**应用内大窗打开与灯箱按用户决策移除**：
  点击一律 openPath 走系统默认程序。
- **文件进便签 V1**：卡片悬停 🔗 按钮（与置顶 ⭐ 并排）-> `copyText`
  （图片 `![](url)`、其余 `[名](url)`）-> 粘贴进便签编辑器即成链接/内嵌图。
...
- **过期提醒**（设置开关 `staleEnabled` + 天数 `staleDays`，默认开/14 天）：mtime 超过
  N 天的文件卡片灰显（opacity+grayscale，仅视觉，功能不受影响）；底栏中簇显示计数
  「⏳ N 项超过 X 天」与「一键归档」按钮；归档抽屉内的文件不参与过期判定。
- **归档**：特殊抽屉「归档」（根目录 `归档/` 文件夹）不进主列表，底栏「归档 ▲」按钮
  自底部**向上展开**面板呈现（`data-drawer="归档"` 使其同时是拖放落点——拖文件上去=
  移入归档）；一键归档 = 全部过期文件批量 moveFiles 到归档（可逆）；面板行内
  「移回（未分类）/删除（系统回收站）」+「清空」。
- **设置面板**（P12 填充）：过期开关/天数；工作区路径（浏览=系统 FolderBrowserDialog
  -> `pickFolder` 回填，应用=`setConfig workspacePath` **热切换**：WorkspacePath 改为
  可写属性，重映射 files.local 虚拟主机 + 重建 watcher + 全量重推）；开机自启
  （HKCU Run 键读写；启动时从注册表回读真实状态推送）。
...
| P12 生命周期 ✅ | 过期灰显 + 计数 + 一键归档（设置开关）· 归档底部向上展开面板 · 设置面板（工作区路径热切换 / 过期天数 / 开机自启，见 §5.9） |
